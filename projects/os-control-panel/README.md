...
The MCP controller classifies actions by risk. Inspection and reversible coordination do not need product approval. `decide_pm_proposal` presents a native Codex form for one sealed proposal revision, while `decide_external_approval` separately covers one exact publication, release, repository, deployment, or visibility action. Starting or resuming the optional API-billed Agents SDK also has its own native prompt. Unsupported, cancelled, malformed, or stale decisions fail closed and retain explicit chat and Streamlit Inbox fallbacks.
